...
def control_loop():
    ...
    with open(CLOCK_FILE, "w") as f:
        json.dump({"date":  datetime.datetime.fromisoformat(glucose_file[-1]["date"][:-2]) + datetime.timedelta(minutes=5) if len(glucose_file) > 0 else datetime.datetime.now(datetime.timezone.utc).isoformat() + "Z"}, f)
